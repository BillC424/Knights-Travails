...
class GameBoard
  ...
  def determine_adjacent_squares(board)
    board.each do |board_square|
      p board_square.board_position
      board_square.adjacent_squares = board_corner_adjacent_square(board_square.board_position) if board_square.board_position == [0, 0] || [7, 0] || [0, 7] || [7, 7]
      p board_square
    end
    board
  end

  def board_corner_adjacent_square(board_square)
   #Left corners
